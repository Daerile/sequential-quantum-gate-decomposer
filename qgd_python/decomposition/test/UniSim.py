...
def perf_collection():
    ...
    for g in results:
        for numdev in results[g]:
            ...
            ax[0].plot(r, [results[g][numdev][x][0] for x in r], label="Qulacs" if numdev==-1 else ("CPU AVX" if numdev==0 else str(numdev) + " " + ("Groq" if g else "DFE")))
            ax[1].plot(r, [results[g][numdev][x][1] for x in r], label="Qulacs" if numdev==-1 else ("CPU AVX" if numdev==0 else str(numdev) + " " + ("Groq" if g else "DFE")))
    ax[0].legend()
    ax[1].legend()
    ...
    for g in results:
        for numdev in results[g]:
            ...
            r = list(sorted(results[g][numdev].keys()))
            ax.plot(r, [results[g][numdev][x][1] for x in r], label="Qulacs" if numdev==-1 else ("CPU AVX" if numdev==0 else str(numdev) + " " + ("Groq" if g else "DFE")))
    ax.legend()
